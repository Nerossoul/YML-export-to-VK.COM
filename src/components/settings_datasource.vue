  ...
  <div>
    <div class="card">
      <div class="card-body">
        ...
        <div v-if="isGettingDataNow && isGettingPhotoBaseNow">
          {{ gettingDataText }}
          <br />
...
export default {
  ...
  data() {
    return {
      ...
      lastProductNumber:
        'https://playavto.ru/vk_export/static/api/lastproduct.json',
      xmlObj: {},
  ...
  watch: {
    xmlObj() {
      ...
      document.title =
        'VK EXPORT:( 0/' + this.$store.state.products.length + ' )';
    }
  },
  computed: {
    categories() {
  ...
  methods: {
    ...
    getLastProductNumber() {
      console.log('getLastProductNumber');
      let fetchOptions = {
        method: 'GET',
        mode: 'cors',
        cache: 'no-cache'
