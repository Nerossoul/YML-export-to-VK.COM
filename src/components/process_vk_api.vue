    ...
    <div class="row mt-2" v-if="show_export_button">
      <div class="col-sm m-3">
        <div class="card">
          ...
          >
            Export products from {{ lastExportedProductNumber }} number of
...
export default {
  ...
  computed: {
    progress_bar() {
      return this.get_prog_bar_string(
        '⣀⣄⣤⣦⣶⣷⣿',
        this.progress_bar_per_cent,
        10,
        ''
      );
    }
  ...
  methods: {
    ...
    isShowContinueButton() {
      return (
        parseInt(this.lastExportedProductNumber) > 0 &&
        parseInt(this.lastExportedProductNumber) <=
      ...
      return new Promise(async (resolve) => {
        ...
        .then((searchResult) => {
          ...
          if (searchResult.response.count === 1) {
            if (
              searchResult.response.items[0].description.indexOf(
                searchString
              ) !== -1
            ) {
            ...
            for (let itemIndex in searchResult.response.items) {
              this.consoleLog(
                searchResult.response.items[itemIndex].description.indexOf(
                  searchString
                )
              );
              if (
            ...
            this.logCurrentAction(
              'Нет в группе. Запуск процесса добавления' +
    ...
    export_AllProductsToVk(useLastExportedProductNumber) {
      ...
      return new Promise(async (resolve) => {
        ...
        for (let product of products) {
          ...
          this.change_title(8);
          let productPreparedData = await this.prepare_ProductData(
            productCounter
          );
          ...
          if (marketItemId !== false) {
            ...
            let response = await this.market_edit(
              marketItemId,
              name,
              description,
              price,
            ...
            if (response.response === 1) {
              ...
              if (
                ...
                this.consoleLog(
                  'you must to delete photos from base. Start fixin this mistake'
                );
                ...
                this.change_title(8);
                productPreparedData = await this.prepare_ProductData(
                  productCounter
                );
                this.consoleLog('Product data prepared. See below');
                this.consoleLog(productPreparedData);
                ...
                let mainPhotoId = productPreparedData.main_photo_id;
                let photoIds = productPreparedData.photo_ids;
                response = await this.market_edit(
                  marketItemId,
                  name,
                  description,
                  price,
            ...
            let AlbumId = this.$store.state.CategoryToAlbumId[
              product.categoryId
            ];
            this.consoleLog('Add this product to album');
            this.logCurrentAction(
              'Дoбавляем данный товар в подбоку' + this.action_string_separator
            );
            let addToAlbumResult = await this.market_addToAlbum(
              marketItemId,
              AlbumId
            );
            if (addToAlbumResult.response === 1) {
              ...
              this.logCurrentAction(
                'Успешно добавлен в подборку!' + this.action_string_separator
              );
            } else if (addToAlbumResult.error.error_code === 1404) {
              this.consoleLog('Already added to this album');
              this.logCurrentAction(
                'Товар уже добавлен в эту подборку' +
                  this.action_string_separator
              );
            } else {
            ...
            let response = await this.market_add(
              name,
              description,
              price,
              mainPhotoId,
            ...
            try {
              ...
              if (response.response.market_item_id) {
                ...
                this.logCurrentAction(
                  'Успешно!' + this.action_string_separator
                );
                await this.pause(this.sleeping_period);
                // eslint-disable-next-line standard/computed-property-even-spacing
                let AlbumId = this.$store.state.CategoryToAlbumId[
                ...
                this.consoleLog('Add this product to album');
                this.logCurrentAction(
                  'Дoбавляем данный товар в подбоку' +
                    this.action_string_separator
                );
                let addToAlbumResult = await this.market_addToAlbum(
                  marketItemId,
                ...
                this.logCurrentAction(
                  'Oшибка: товар не добавлен!' + this.action_string_separator
                );
              }
            } catch (error) {
              ...
              console.error(error);
              this.logCurrentAction(
                'Критическая ошибка: ' + this.action_string_separator
              );
            }
          }
          this.change_title(100);
    ...
    get_photo_id(fileLink, vendorCode) {
      return new Promise(async (resolve) => {
        ...
        if (
          ...
          this.$store.state.photoBase[vendorCode][fileLink]
        ) {
        ...
        if (photoId === '') {
          photoId = await this.upload_photoToVkGroupCommonMethod(
            fileLink,
            vendorCode
          );
        }
    ...
    prepare_ProductData(arrayIndex) {
        ...
        if (Array.isArray(productPhotoUrls)) {
          for (let item of productPhotoUrls) {
            ...
            if (photoCounter === 1) {
              ...
              photoIds +=
                separator + (await this.get_photo_id(item, vendorCode));
            } else {
      ...
      Ссылка на сайт: ` +
          productSiteUrl +
          '?utm_source=vk.com&utm_medium=group_shop&utm_campaign=vk.com_group_shop&utm_content=product_' +
          vendorCode +
          `
      ОПИСАНИЕ ТОВАРА:
      ` +
          description;
        let data = {
          name: this.getProductNameBefore100Chars(
            this.prepared_product_data.name
          ),
          description: description,
          category_id: 205,
        ...
        this.logCurrentAction(
          'Запрос сформирован' + this.action_string_separator
        );
        return data;
      });
    },
    ...
    upload_photoToVkGroupCommonMethod(fileLink, vendorCode) {
      ...
      return new Promise(async (resolve) => {
        ...
        .catch(async (error) => {
          ...
          this.logCurrentAction(
            this.action_string_separator +
              'Ошибка загрузки фото. Пропускаем данную ссылку.' +
              this.action_string_separator
          );
    ...
    _onwApi_call(method, params) {
      ...
      return fetch(url, {
        ...
        .then(async (json) => {
          ...
          if (this.isResponseHasCaptcha(json)) {
            // capcha handling
    ...
    get_prog_bar_string(barString, perCent, bars, sep = ' ') {
      ...
      if (barPart !== 0) {
        ...
        let partCharIndex = Math.round(
          ((barString.length - 1) / partMax) * currentParts
        );
